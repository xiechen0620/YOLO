...
import numpy as np
from keras.layers import convolutional, LeakyReLU, BatchNormalization, Input, Concatenate, Lambda, Add
...
def conv2d_with_bn(filters, kernel_size, stride, padding, activation, bias, batch_normalize):
    ...
    def layer_func(input):
        ...
        conv_layer = convolutional.Conv2D(filters=filters, kernel_size=kernel_size,
                                          strides=stride, padding=pad, activation=active, use_bias=bias)
...
def load_weights(weightfile, blocks):
    """
    ...
    # Note: for now, I can't get layer type when travorse the model.layers
    #       maybe I could get it through this: 
    #       https://taizilongxu.gitbooks.io/stackoverflow-about-python/content/56/README.html
    ...
    for i in range(len(blocks[1:])):
        ...
        # Otherwise ignore.
        # Note: if use generator to load weights from file, I could use generator.send()
        #       to send new value(read length) into generator.
        if module_type == "convolutional":
            if blocks[i+1]["batch_normalize"] == 1:
                # With BN layer
                # BN layer weights' order is [gamma, beta, mean, std]
                # Qoute: https://github.com/keras-team/keras/issues/1523
                pass
            else: 
                # Without BN layer.
                pass
    ...
    for index, block in enumerate(blocks[1:]):
        if block["type"] == "convolutional":
            ...
            layer = Lambda(bilinear_upsample(stride=stride), output_shape=bilinear_upsample_shape(stride=stride),
                           name="bilinear_upsample"+str(index))
...
def main():
    ...
    # Weights of model have been initialized.
    # We could call model.get_weights() at here.
    # So, may be I could get model weights' shape and load them from file
    # before compile model.
    ...
    #     print(yolo_model.layers[layer_index+1].get_config())
    #     weight = yolo_model.layers[layer_index+1].get_weights()
    #     print(weight[0].shape)
    # test_img = get_test_image()
    # print(type(test_img))
    # test = yolo_model.predic(test_img)
