...
def predict_transform(predetection, inp_dim: int, anchors, num_classes):
    """
    ...
    :param predetection: a 'channel last' tensor, which means that shape of our output from convNet is (B*H*W*C)
    ...
    predict_shape = predetection.shape
    ...
    new_predetection = predetection.reshape(batch_size, grid_size * grid_size,
    ...
    new_predetection = new_predetection.reshape(batch_size, grid_size * grid_size * num_anchors,
    ...
    new_predetection[:, :, 0] = tf.sigmoid(new_predetection[:, :, 0])
    new_predetection[:, :, 1] = tf.sigmoid(new_predetection[:, :, 1])
    new_predetection[:, :, 4] = tf.sigmoid(new_predetection[:, :, 4])
    ...
    new_predetection[:, :, :2] += x_y_offset_tensor
    ...
    new_predetection[:, :, 2:4] = tf.exp(new_predetection[:, :, 2:4]) * anchors_tensor

    # apply sigmoid activation to the class scores.
    new_predetection[:, :, 5: (5 + num_classes)] = tf.sigmoid(new_predetection[:, :, 5:(5 + num_classes)])

    # resize the detections map to the size of input image size.
    new_predetection[:, :, :4] *= stride

    # return transfored detection
    return new_predetection
